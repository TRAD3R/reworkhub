...
//    private $adminChats = [583732141]; // admin's chats array
    ...
    public static function getChannelView(Job $job)
    {
        ...
        $companyTitle =  !empty($job->company_title) ? "<bold>" . Yii::t('app', "PH_COMPANY") . ":</bold>" . PHP_EOL . self::replaceHTML($job->duties) : "";
        ...
        $duties =  !empty($job->duties) ? "<bold>" . Yii::t('app', "DUTIES") . "</bold>" . PHP_EOL . self::replaceHTML($job->duties) : "";
        $view = str_replace("[JOB_DUTIES]", $duties, $view);
        $requirements = !empty($job->requirements) ? "<bold>" . Yii::t('app', "REQUIREMENTS") . "</bold>" . PHP_EOL . self::replaceHTML($job->requirements) : "";
